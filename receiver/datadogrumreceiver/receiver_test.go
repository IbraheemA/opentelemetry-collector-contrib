...
func TestDatadogServer(t *testing.T) {
	...
	dd, err := newDataDogRUMReceiver(
		cfg,
		receivertest.NewNopSettings(),
	)
	dd.(*datadogRUMReceiver).nextTracesConsumer = consumertest.NewNop()
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			req, err := http.NewRequest(
				http.MethodPost,
				fmt.Sprintf("http://%s/v0.7/traces", dd.(*datadogRUMReceiver).address),
				tc.op,
			)
...
func TestDatadogMetricsV1_EndToEnd(t *testing.T) {
	...
	require.NoError(t, err, "Must not error when creating receiver")
	dd.(*datadogRUMReceiver).nextTracesConsumer = sink
	...
	req, err := http.NewRequest(
		http.MethodPost,
		fmt.Sprintf("http://%s/api/v1/series", dd.(*datadogRUMReceiver).address),
...
func TestDatadogMetricsV2_EndToEnd(t *testing.T) {
	...
	require.NoError(t, err, "Must not error when creating receiver")
	dd.(*datadogRUMReceiver).nextMetricsConsumer = sink
	...
	req, err := http.NewRequest(
		http.MethodPost,
		fmt.Sprintf("http://%s/api/v2/series", dd.(*datadogRUMReceiver).address),
